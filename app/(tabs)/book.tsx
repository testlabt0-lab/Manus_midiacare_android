...
export default function BookingScreen() {
  ...
  useEffect(() => {
    ...
    void loadBookingDraft().then((draft) => {
      ...
      setStep(draft.step); setService(draft.service); setClinic(draft.clinic); setAddress(draft.address); setScheduledAt(new Date(draft.scheduledAt));
  ...
  useEffect(() => {
    ...
    void saveBookingDraft({ step, service, clinic, address, scheduledAt: scheduledAt.toISOString() });
  }, [address, clinic, draftReady, scheduledAt, service, step]);
  ...
  const back = async () => {
    ...
    await clearBookingDraft(); router.back();
  };
  ...
  return <ScreenContainer><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><Pressable onPress={() => void back()} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><MaterialIcons name="arrow-forward" size={22} color="#0B776B" /></Pressable><View style={styles.headerCopy}><Text style={styles.title}>حجز زيارة منزلية</Text><Text style={styles.subtitle}>أكمل الخطوات الأربع لإرسال طلبك للعيادة.</Text>{draftReady ? <Text style={styles.draftHint}>تُحفظ هذه المسودة محلياً وليست موعداً مؤكداً.</Text> : null}</View></View>
